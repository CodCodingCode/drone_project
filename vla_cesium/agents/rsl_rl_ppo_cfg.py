"""PPO config for the Cesium VLA env — identical to vla/agents/rsl_rl_ppo_cfg.py
except the experiment name, so checkpoints and TensorBoard logs go to a
separate `vla_drone_cesium/` subdirectory.
"""
...
@configclass
class VLACesiumPPORunnerCfg(RslRlOnPolicyRunnerCfg):
    num_steps_per_env = 4
    max_iterations = 5000
    save_interval = 100
    experiment_name = "vla_drone_cesium"
    logger = "tensorboard"
...
VLADronePPORunnerCfg = VLACesiumPPORunnerCfg
