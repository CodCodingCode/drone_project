...
Launch:
    ...
    ./isaaclab.sh -p /home/ubuntu/drone_project/lang_nav_siglip/play.py
...
_DEFAULT_CKPT = os.path.join(
    _DRONE_PROJECT,
    "logs", "rsl_rl", "lang_drone_siglip",
    "latest", "model_2999.pt",
)
...
import lang_nav_siglip  # noqa: F401 — registers Isaac-LangDroneSigLIP-Direct-v0
...
def main():
    ...
    video_dir = os.path.join(_DRONE_PROJECT, "videos", "lang_nav_siglip")
    os.makedirs(video_dir, exist_ok=True)

    # Clean up old lang nav siglip playback videos
    for old in glob.glob(os.path.join(video_dir, "lang_nav_siglip_playback-*.mp4")):
        os.remove(old)
    ...
    # Create environment
    if args_cli.video:
        env = gym.make("Isaac-LangDroneSigLIP-Direct-v0", cfg=env_cfg, render_mode="rgb_array")
    else:
    ...
    if args_cli.video:
        env = gym.wrappers.RecordVideo(
            env,
            video_dir,
            name_prefix="lang_nav_siglip_playback",
